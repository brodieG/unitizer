...
You don't need to go through the entirity of this vignette to start using `unitizer`.  The `unitizer` interactive environment strives to be self explanatory, and provides contextual help.  You can always type `H` at the `unitizer` prompt to receive assitance.
...
As noted previously simple assignments are not considered tests.  They are stored in the `unitizer` store, but you are not asked to review them.  The implicit assumption is that if there is an assignment the intent is to use it as an input to another test that will be reviewed, so skipping assignment review saves some unnecessary user interaction.
...
One point worth highlighting: the `unitizer` prompt **is complex and behaves differently from the standard R prompt in some suble ways**.  Some examples:
* Even though you can see objects produced by tests, you can't actually remove them
...
* Other subtle issues discussed at length in the "Esoteric Topics" section
...
Before `unitizer` quits completely we are given the chance to review are changes by using the `B` or `R` navigation commands, although this only happens if you have made changes to the `unitizer` through the interactive prompting, or if the test evaluate in fewer seconds than `getOption("unitizer.prompt.b4.quit.time")` (ten seconds as of the time of this writing).  In this case we just quit without further ado; since we had not chosen any of the new test values to replace the old there was no need to modify the unitizer store.  Note that in practice this sequence would not have prompted us before exiting `unitizer` because it actually ran in less than ten seconds, but we show it here for documentation purposes.
...
As a work-around to this problem `unitizer` only keeps the environment associated with the actual reference test when you type `N` at the `unitizer` prompt.  `unitizer` then grafts that test to the environment set associated with the new test runs.  This means that in future `unitizer` runs the reference objects we examine when reviewing this test may not be the same objects that existed when the test itself was evaluated.  The unitizer `ls` command will highlight which objects have this problem.
