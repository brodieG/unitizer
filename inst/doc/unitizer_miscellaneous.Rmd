...
recommend you checkout your most recent version of the rdses that are part of
the pull request.  You can then re-run the tests and review what each of them
does.
...
See this [SO Q/A](http://stackoverflow.com/questions/20572288/capture-arbitrary-conditions-with-withcallinghandlers) for more details on the problem.
...
* `quit` and `q` are wrappers around the base functions that allow `unitizer` to
  quit gracefully
* `traceback` while not replaced, is supported by a fair bit of voodoo involving
  among other things assigning to `base::.Traceback`
