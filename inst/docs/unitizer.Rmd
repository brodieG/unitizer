...
At this point, `unitizer` starts showing you the results of evaluating the tests.  Note that assignment expressions are not considered tests; they are stored, but you are not asked to review them, and their values will not be compared to future evaluations.  The implicit assumption is that if there is an assignment the intent is to use it as an input to another test, so skipping assignment review saves some unnecessary user interaction.
...
Looks good.  So let's accept the test by typing in `Y` and hitting ENTER:
...
So the first test, `(res <- fastlm(x, y))` is now queued for storing, and we're prompted about the second test.  We will also accept this one, as well as the next 3, which brings us to:
...
The `.rds` file contains all the tests.
...
By comparing the output of `.new` and `.ref` we can see that something went wrong with our `intercept` calculation. We could also have re-evaluated our test instead of typing `.new` at the prompt, but if the computation is slow using `.new` avoids recomputation.  Also, the only way to get the original value is with `.ref`.

We now know what went wrong with our test, so we will reject this test as well as the subsequent related test (`getIntercept` will obviously fail too) which takes us to our error causing tests:
...
This is telling us that our tests failed because they did not produce the same error message.  We can check with `getConds` (note `getConds` returns a list of conditions):
...
The remaining two failed tests also failed for similar reasons, so we will accept them too, which again takes us to the final prompt:
    ...
    You are about to IRREVERSIBLY:
    - Replace 2 out of 4 failed tests
    unitizer> Y
    unitizer updated

Our `unitizer` `.rds` is now updated.  We replaced the two error tests with their new versions, but did not replace the intercept tests since we know those are wrong.
...
Let's go back to our test review when our new version of `fastlm` was returning incorrect values.
