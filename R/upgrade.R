    ...
    if(inherits(try(validObject(object, complete=TRUE)), "try-error")) {
      if(
        ...
        message(
          ...
          packageVersion("unitizer"),"). Do you wish to OVERWRITE ",
          "EXISTING `unitizer` store with an upgrade version ([Y]es, [N]o)?"
        )
        help <- paste0(
          "Pressing Y will upgrade your `unitizer` to the newest version if ",
          "possible and replace the existing store."
        )
        act <- unitizer_prompt(
        ...
        msg <- paste0(
          ...
          "corresponding store, and re-run the tests with the new version of `unitizer`."
        )
        if(identical(act, "Y")) {
          if(
