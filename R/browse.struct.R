...
setClass("testorBrowse", contains="testorList",
  slots=c(
    mapping="data.frame",      # this should probably be an S4 class given the validation...
  ...
  prototype=list(
    mapping=data.frame(
      item.id=integer(),          # unique, 1 incrementing up to total number of reviewable items
      item.id.rel=integer(),      # non-unique, unique within each sec/sub.sec
      sec.id=integer(),           # unique
      sub.sec.id=integer(),       # unique
      reviewed=logical(),         # What review action user took; NA for tests that haven't been reviewed yet
      review.val=chacter(),       
  ...
  validity=function(object) {
    if(
      !identical(
        ...
        c("item.id", "item.id.rel", "sec.id", "sub.sec.id", "reviewed", "ignored")
      ) || !is.integer(unlist(object@mapping[1L:3L])) ||
      !is.logical(object@mapping[[4L]]) ||
      !is.character(object@mapping[[5L]]) || !is.logical(object@mapping[[6L]]) 
    ) {
      return(
        "slot `@mapping` must be a data.frame with colnames: ",
        "c(\"item.id\", \item.id.rel\", \"sec.id\", \"sub.sec.id\", ",
        "\"reviewed\", \"review.val\", \"review.type\", \"logical\") ",
        "and cols 1:4 integer, col 5 logical, col 6 character, and col 7 logical." 
) } } )
...
setMethod("show", "testorBrowse", function(x, ...) {
  ...
  out <- character()
  disp.len <- getOption("width") - 11L - 4L - max(nchar(x@mapping@item.id))
...
setClass("testorBrowseSubSection",
  ...
  validity=function(object) {
    if(
      ...
      return("Argument `reviewed` must be a 1 length logical")
...
setClass("testorBrowseSubSectionNew", contains="testorBrowseSubSection",
  slots=c(
    ...
    actions=actions=c(Y="A", N="C"), show.out=TRUE
) )
