...
#'   the files therein into an environment that has for parent \code{par.env}.
#'   The objects you create/pass will be visible to the tests, but you will not
#'   be able to list them with \code{ls}, etc (Note this behavior is being
...
unitize <- function(
  ...
  pre.load=file.path(dirname(test.file), "helper")
) {
