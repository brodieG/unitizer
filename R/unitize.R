...
#'       \code{\link{sys.source}}s the files therein into an environment that has
#'       for parent \code{par.env}
...
#'   environments the tests are run in.  Objects generated by this process are
#'   not recorded by \code{unitizer}, so any changes to them may cause tests to
#'   fail in a difficult to track down manner.  For this reason we encourage you
#'   to use this feature sparingly.  Any packages you load in these files
#'   will be unloaded upon completion of the \code{unitize} process unless you
#'   modify the \code{search.path.clean} setting.
