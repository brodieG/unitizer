...
## generated by `trace_at_end` that does not return control.
##
##
...
trace_at_end <- function(fun, tracer, print, where) {
  ...
  trace_editor <- function(name, file, title) {
    ...
    # Generate a forwarding call that calls the inner function with every
    # arg of the outer functions

    fwd.call <- as.call(
      c(list(NULL),
      sapply(names(formals(name)), as.name, simplify=FALSE))
    )
    fwd.call[[1L]] <- f.copy
    ...
    body(f.fin) <- bquote({
      .res <- try(withVisible(.(fwd.call)), silent=TRUE)
      .doTrace(.(tracer))
