...
#'   \item \code{unitizer.show.diff} TRUE or FALSE, whether to show a diff of
...
validate_options <- function(opts.to.validate, test.files=NULL) {
  ...
  with(
    opts.to.validate,
    {
      ...
      if(!is.TF(unitizer.show.diff))
        stop("Option `unitizer.show.diff` must be TRUE or FALSE")
      if(
