...
runtests <- function(test.file, store.id=sub("\\.[Rr]$", ".rds", test.file)) {
  ...
  if(identical(testor, FALSE)) {
    ...
    if(
      ...
      message(
        "You are attempting to load a `testor` generated by an older version (", 
        testor@version, ") of `testor` vs. the currently loaded one (", packageVersion("testor"), 
        "). Do you wish to attempt to upgrade your `testor` to the new version ([Y]es, [N]o)?"
      )
      ...
      msg <- paste0(
        ...
        "run the tests with the version of testor you used to generate them in ",
        "the first place to ensure they still pass, manually delete the ",
        "corresponding RDS, and re-run the tests with the new version of `testor`."
      )
      if(identical(act, "Y")) {
        if(
          inherits(try(testor <- upgrade(testor)), "try-error") ||
          inherits(try(validObject(testor, complete=TRUE)), "try-error")
        ) {
