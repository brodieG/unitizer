...
One shortcoming of `unitizer` is that it is not conducive to either self documentation or to test driven / [extreme](http://www.extremeprogramming.org/rules/testfirst.html) programming.  If these features are important to you then you are likely better off using more formal unit testing frameworks.

**UPDATE**: recently `testthat` added `expect_equal_to_reference`, which takes a similar approach by storing test results in an RDS file and subsequently comparing to the pre-calculated value.  The difference is that with `expect_equal_to_reference` more user work is required since each test requires a separate file, and both the initial result verification and checking differences requires manual review and/or manually loading the RDS file for comparison.  Additionally, it appears conditions still need to be handled independently.

#### Converting `testthat` tests to `unitizer`

If you have a stable set of tests it is probably not worth trying to convert them to `unitizer` unless you expect the code those tests cover to change substantially.  If you do decide to convert tests you can use that provided `testthat_translate*` functions (see `?testthat_translate_file`).
...
Objects in `.GlobalEnv` will not be visible, and any non basic packages will be detached prior to test evaluation (you can re-attach those you need in your tests with `library`).  The search path will be restored to its original state after `unitizer` completes execution.  For more details see the [reproducible tests vignette](vgn05reproducibletests.html).
...
### Tests Involving Reference Objects May Be Confusing

`unitizer` evaluates all tests first, and only then does it prompt you for review if it is required.  Each time you review a test, `unitizer` recreates that test's environment as it was when it was first run
 (see [subtleties of interactive environment](vgn06nittygritty.html#subtleties-of-the-interactive-environment)).  This works most of the time, except that reference objects will always reflect the latest modifications made to them in testing.  In other words, the value reference objects have during review may not be the value they had when the test was evaluated, which can be confusing if you do not realize what is happening.  We are considering ways to address this issue.
