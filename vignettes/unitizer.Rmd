...
* "Unitizer and Version Control": Some thoughts on how to handle binary files
...
You don't need to go through the entirety of vignette to start using `unitizer`.  The `unitizer` interactive environment strives to be self explanatory, and provides contextual help.  You can always type `H` at the `unitizer` prompt to receive assitance.
...
Anscillary packages used for examples and tests are in `inst/example.pkgs` in source, so in order to install on your system you can use (after installing `unitizer`):

```
unitizer.path <- system.file(package="unitizer")
library(devtools)
install(paste0(unitizer.path, "/example.pkgs/unitizerdummypkg1"))
install(paste0(unitizer.path, "/example.pkgs/unitizerdummypkg2"))
install(paste0(unitizer.path, "/example.pkgs/fastlm"))
```
...
It is possible to have nested sections, though at this point in time `unitizer` only reports information at the outermost sectin level.
