...
* [`vignette("unitizer_index")` for a list of vignettes](unitizer_index.html)
...
### And Manipulates the Search Path

`unitizer` tracks and manages many aspects of state to make your tests more reproducible.  For example, `unitizer` will set your search path to what is found in a vanilla R session prior to running tests so that you can ensure your test is not affected by libraries you happen to have loaded in your session (the corollary to this is you must explicitly load libraries your tests need from within your tests). The following aspects of state are actively tracked and managed:
...
Additionally, `unitizer` is able to control global options and namespaces, though these last two features are disabled by default.  For more details and on for instructions on how to modify this behavior, see `?unitizerState` and the [reproducible tests vignette](unitizer_reproducible_tests.html).

### Some Base Functions are Modified

`unitizer` traces some functions during `unitize` evaluation.  The functions are
then untraced `on.exit`, so the only R evaluations that are affected are test
expressions that are `unitized`.

`library`, `require`, `attach`, and `detach` are traced to implement search path tracking.  The `trace` is only used for internal logging and should be completely transparent to users.

`q` and `quit` are traced to give the user an opportunity to cancel the quit
action in case they meant to quit from `unitizer` instead of R.  Use Q to
quit from `unitizer`, as you would from `browser`.

`ls` is overwritten with a specialized version for use in `unitizer`.  You can
still access the original function with `base::ls`.

See [esoteric topics vignette](unitizer_miscellaneous.html).
...
* Some base functions are [modified](#some-base-functions-are-modified)
