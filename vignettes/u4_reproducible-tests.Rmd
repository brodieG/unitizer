...
`options(unitizer.state='recommended')`.  Be sure to read `?unitizerState`
before you enable this setting as there are cases when state management may not
work.
...
In the "recommended" state tracking mode `unitizer` runs on a "trimmed" search
