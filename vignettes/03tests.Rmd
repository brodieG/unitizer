...
As noted previously simple assignments are not considered tests.  They are stored in the `unitizer` store, but you are not asked to review them, and their values are not be compared with existing reference values prior to storage.  The implicit assumption is that if there is an assignment the intent is to use the resulting object in some later test at which point any issues will crop up.  Skipping assignment review saves some unnecessary user interaction.
...
If the `unitize` is run with `sdtderr` sunk, then it will not capture any `stderr` produced by the test expressions.  If one of the test expressions enables `stderr` capture, then `unitize` will stop capturing messages from that point on.  If after this another test expression disables message capture, this function will start capturing messages starting with the subsequent test expression.
