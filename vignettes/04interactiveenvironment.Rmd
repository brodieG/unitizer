---
title: "The Interative Environment"
...
Both these functions use the same interactive environment, though rules therein are slightly different.  For example, in `review`, all the tests are considered passed since there is nothing to compare them to, and the interactive environment will step you through all the passed tests.  `unitize` will normally not show you any of the passed tests.

We will focus on `unitize` for the rest of this vignette since most of the commentary about that applies equally to `review`.
...
Note that tests are displayed in the order they appear in the test file, not in
the order they would be reviewed in, which is why the test numbers are not
necessarily sequential (see vignette for details and exceptions).

What test do you wish to review (input a test number, [U]nreviewed)?
unitizer>
```
The `[R]eview` option produces a list of all the tests in the order in which they appear in the test file.  You can type the number of a test to review it, or U to go to the first test that hasn't been reviewed (more on this in a minute).  Let's examine one of the lines in more detail:
```
     3. get_slope(res) .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  Failed:-
     ^   ^                                                                ^   ^
     |   |                                                                |   |
...
You can jump ahead to any test from the review menu, even tests that are typically not reviewed (i.e. ignored or , though if you go to those you will be brought back to the review menu once you complete the review.  This is because those tests are not part of the normal review flow.
...
* Even though you can see objects produced by tests, you can't actually remove them with `rm`
* Any objects you create at the prompt are only available for the test you are currenlty reviewing, so don't expect them to still be there at subsequent prompts
* Other subtle issues discussed at length in the ([nitty gritty vignette](06nittygritty.html))
* There are special `unitizer` objects `.new` and `.ref` that let you review the results of tests (we will discuss these next)
* `ls` is over-ridden by a special `unitizer` version (we will also dicuss this next)

### `.new` and `.ref`

As we saw in the demo there are special objects available at the prompt: `.new`, and for all but new tests, `.ref`.  These objects contain the values produced by the newly evaluated test (`.new`) and by the test when it was previously run and accepted (`.ref`).  `.new` might seem a bit superfluous since the user can always re-evaluate the test expression at the `unitizer` prompt to review the value, but if that evaluation is slow you can save a little time by using `.new`.  `.ref` is the only option you have to see what the test used to produce back when it was first accepted into the `unitizer` store.
...
* `getOut(.new)` returns the screen output captured during test evaluation (note often this will be similar to what you get from `.new` or `getVal(.new)` since typing those expressions at the prompt leads to the value being printed)
...
* `getAborted(.new)` returns whether the test expression invoked an "abort" restart (e.g. called `stop` at some point)
...
[1] "res" "x*"   "y'"
```
where object names have symbols such as `*` or `'` appended to them.  This happens because `unitizer` does not store the entire environment structure of the reference tests.  In some cases, some objects may no longer be available (`*`), may no longer be the same as they were when the reference test was evaluated (`'`), or may actually be present but were not when the reference test was evaluated (`**`).  For more discussion see `?healEnvs` and the [nitty gritty vignette](06nittygritty.html).
