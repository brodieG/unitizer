...
vignette: >
  %\VignetteIndexEntry{06 - Nitty Gritty Details}
...
It turns out the above example actually works because as noted in [INSERT LINK TO TESTS VIGNETTE], environments are only defined for tests, an neither the `a` or `b` assignments are tests, so both `a` and `b` are assigned to the environment
...
* All attached packages and environments other than those that are pre-loaded with a clean R session are detached (see [LINK TO REPRODUCIBLE TESTS VIGNETTE]).
