...
When running `unitize` with `search.path.clean=TRUE`, `unitizer` will restore the original search path by using `library/attach` on any previously detached objects or packages.  This generally works fine with the notable exception of any objects that contain environment references to the search path object itself.
