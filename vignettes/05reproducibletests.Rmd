...
By default `unitize` will run tests in a clean environment that does not have for ancestor `.GlobalEnv`.  This means that objects in the global environment / workspace will not affect your tests.  If you prefer to run your tests in `.GlobalEnv` you can use:
```
unitize(..., env.clean=.GlobalEnv)
```
The parent of the environment `unitizer` runs its tests in is the last environment on the search path before `.GlobalEnv`.  Note however that `unitizer` temporarily modifies the search path during test evaluation / review (see next section).
...
There are a few possible solutions to this problem.  For now we have adopted the simplest which is to keep the `tools:rstudio` environment on the search path even with `search.path.clean=TRUE` (you can over-ride this by changing `search.path.keep`).  Other options include re-attaching with `parent.env<-` instead of `attach`, but messing with the search path in that way seems to be exactly what R core warns about in `?parent.env`:
