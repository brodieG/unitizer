...
`unitize` will step through your tests in an interactive environment so that you can review them before they are stored as reference values.  That's it.  Any time you make changes to your project, you can just re-run `unitize` and it will alert you of any regressions.
...
* Tests are easy to write
    * Notice how we don't need to provide the result vector
...
* The tests are self documenting
    * Expected results are obvious from the source, whereas it isn't so with `unitizer`
* Once you write the test you're done
    * With `unitizer` you still need to `unitize` and review them
...
```
The path specification for test files should be relative to the `tests` directory as that is what `R CMD check` effectively sets the working directory to before running the files in `tests/`.  This means you can't just source your `run.R` file without also first setting the working directory to `tests/`.
...
* We provide modified versions of `quit`/`q` and `ls` (see [nitty gritty vignette](06nittygritty.html)) at the `unitizer` prompt
