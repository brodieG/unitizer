...
local({
  ...
  if(
    ...
    stop(
      "Packages ", cc(deparse(names(tmp.pkgs)[which.inst], width.cutoff=500)),
  ...
  pkg.inst <- try(install.packages(pkg.dirs, repos=NULL, lib=lib, type="src"))
  ...
  on.exit({
    ...
    remove.packages(tmp.pkgs, lib=lib)
  )
