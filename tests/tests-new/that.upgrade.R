...
#     expect_warning(unitizer.up <- unitizer:::upgrade_internal(unitizer), 
#         "Slot .* does not exist")
...
unitizer <- get_unitizer("helper/trivial.unitizer.0.4.2")
# expect_error(validObject(unitizer, complete = TRUE))
validObject(unitizer, complete = TRUE)
# expect_equal(as.character(unitizer@version), "0.4.2")
...
#     expect_error(capture.output(unitizer:::renameSlot(x, "c", 
#         "b"), type = "message"), "Old slot `c` doesn't")
...
# expect_error(validObject(x))
validObject(x)
# expect_error(capture.output(unitizer:::renameSlot(x, "c", "b"), 
#     type = "message"), "Old slot `c` doesn't")
capture.output(unitizer:::renameSlot(x, "c", "b"), type = "message")
x.rename <- unitizer:::renameSlot(x, "a", "b")
# expect_true(validObject(x.rename))
...
#     unitizer.rds <- readRDS(file.path(tmpdir, "tests.unitizer", 
#         "data.rds"))
...
unitizer.rds@eval.time <- runif(5)
# expect_match(unitizer:::unitizer_valid(unitizer.rds), "NB: ")
unitizer:::unitizer_valid(unitizer.rds)
