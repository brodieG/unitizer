...
# No unreviewed tests
unitizer:::read_line_set_vals(c("Y", "Q", "U", "Y", "B", "U", 
    "Y", "Y", "U", "B", "U", "Q"))
txt0 <- unitizer:::capture_output(unitize(.unitizer.test.file, 
...
expect_equal_to_reference(unitizer:::clean_eval_exp(txt0), file.path("helper", 
    "refobjs", "unitize2_unreview.rds"))
# Test re-eval
...
unitizer:::read_line_set_vals(c("1", "Y", "R", "Y", "Q", "2", 
    "Y", "Y", "3", "Y", "RR", "Y", "Q", "Q"))
txt1a <- unitizer:::capture_output(untz1 <- unitize_dir(test.dir, 
    interactive.mode = TRUE))
txt1b <- unitizer:::capture_output(print(untz1))
# temp file names
untz1.clean <- lapply(untz1, `attributes<-`, NULL)
# test_that("Re-eval", {
...
expect_equal_to_reference(txt1b, file.path("helper", "refobjs", 
    "unitize2_rerun_b.rds"))
expect_equal_to_reference(untz1.clean, file.path("helper", "refobjs", 
    "unitize2_rerun_res.rds"))
...
out.1 <- unitizer:::capture_output(unitize(test.file.1, test.store, 
    auto.accept = "new"))
unitizer:::read_line_set_vals(c("B", "Q"))
out.2 <- unitizer:::capture_output(untz.2 <- unitize(test.file.2, 
    test.store, interactive.mode = TRUE))
...
unitizer:::capture_output(unitize(file.path(temp.loc, "sects.R"), 
    auto.accept = "new", interactive.mode = FALSE))
...
cat("\n", file = temp.bad)
badname.capt <- suppressWarnings(unitizer:::capture_output(unitize(temp.bad)))
unlink(temp.empty)
...
# expect_true(sum(grepl("Unable to find test", reeval.capt$message)) == 
#     1L)
sum(grepl("Unable to find test", reeval.capt$message)) == 1L
...
# state = environment() so we can access variables from this local
unitizer:::capture_output(unitize(temp.cond, state = environment(), 
    interactive.mode = TRUE))
cond.message <- "goodbye world"
unitizer:::read_line_set_vals("Q")
cond.capt <- unitizer:::capture_output(unitize(temp.cond, state = environment(), 
    interactive.mode = TRUE))
...
# expect_true(sum(grepl("Unable to compare value", bad.comp.capt$message)) == 
#     1L)
sum(grepl("Unable to compare value", bad.comp.capt$message)) == 
    1L
# expect_true(sum(grepl("Corrupted", bad.comp.capt$output)) >= 
#     1L)
...
# expect_error(capture.output(unitize_dir(test.dir, store.ids = function(x) stop("Bad store map fun")), 
#     type = "message"), "attempting to use it to convert test file")
capture.output(unitize_dir(test.dir, store.ids = function(x) stop("Bad store map fun")), 
    type = "message")
