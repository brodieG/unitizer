...
#     old.seed <- if (!exists(".Random.seed")) 
#         NULL
#     else .Random.seed
#     seed.dat <- getOption("unitizer.seed")
#     suppressWarnings(untz.glob <- unitizer:::unitizerGlobal$new(enable.which = setNames(2L, 
#         "random.seed")))
...
#     untz.glob$resetFull()
#     if (is.null(old.seed)) 
#         expect_false(exists(".Random.seed"))
#     else expect_identical(old.seed, .Random.seed)
# })
# - "Random Seed" --------------------------------------------------------------
...
.Random.seed
untz.glob$resetFull()
if (is.null(old.seed)) expect_false(exists(".Random.seed")) else expect_identical(old.seed, 
    .Random.seed)
# test_that("State Show", {
#     expect_equal(capture.output(show(unitizer:::unitizerStatePristine())), 
#         c("           Settings Values", "1           par.env <auto>", 
#             "2       search.path      2", "3           options      2", 
#             "4 working.directory      2", "5       random.seed      2", 
#             "6        namespaces      2", "-----", "0: off", 
#             "1: track starting with initial state", "2: track starting with clean state", 
...
#         "2       search.path      2", "3           options      2", 
#         "4 working.directory      2", "5       random.seed      2", 
#         "6        namespaces      2", "-----", "0: off", "1: track starting with initial state", 
#         "2: track starting with clean state", "<auto>: use special unitizer environment as 'par.env'", 
#         "See `?unitizerState` for more details."))
...
#     expect_equal(all.equal(blah, dummy), "`.NEW` value was not recorded, but `.REF` value was; they are likely different")
#     expect_equal(evalq(all.equal(new("unitizerDummy"), "hello"), 
#         getNamespace("stats")), ref.txt)
#     expect_equal(evalq(all.equal("hello", new("unitizerDummy")), 
#         getNamespace("stats")), c("Modes: character, S4", "Attributes: < target is NULL, current is list >", 
...
state.H <- new("unitizerGlobalState", options = list(a = structure(FALSE, 
    class = "unitizer_glob_state_test"), b = 2))
# test_that("as.state", {
#     expect_identical(unitizer:::as.state("recommended"), unitizer:::as.state(unitizer:::unitizerStateSuggested()))
#     expect_identical(unitizer:::as.state("suggested"), unitizer:::as.state(unitizer:::unitizerStateSuggested()))
#     expect_identical(unitizer:::as.state("pristine"), unitizer:::as.state(unitizer:::unitizerStatePristine()))
#     expect_equal(unitizer:::as.state(.GlobalEnv), unitizer:::as.state(unitizer:::unitizerStateSuggested(par.env = .GlobalEnv)))
#     expect_equal(unitizer:::as.state(in_pkg("stats")), unitizer:::as.state(unitizer:::unitizerStateSuggested(par.env = getNamespace("stats"))))
#     stats.lib <- file.path(system.file(package = "stats"), "R")
#     expect_equal(unitizer:::as.state(in_pkg(), test.files = stats.lib), 
#         unitizer:::as.state(unitizer:::unitizerStateSuggested(par.env = getNamespace("stats"))))
#     expect_error(unitizer:::as.state(200))
#     state <- unitizer:::unitizerStateOff()
#     state@options <- 2L
#     expect_error(validObject(state))
#     expect_identical(unitizer:::as.state(unitizer:::unitizerStateRaw()), 
#         unitizer:::unitizerStateProcessed())
#     expect_identical(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = "stats")), 
#         unitizer:::unitizerStateProcessed(par.env = getNamespace("stats")))
#     in.pkg.state <- try(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg()), 
#         test.files = getwd()))
#     if (inherits(in.pkg.state, "try-error")) {
#         warning("in_pkg() test did not work")
#     }
#     else {
#         expect_identical(in.pkg.state, unitizer:::unitizerStateProcessed(par.env = getNamespace("unitizer")))
#     }
#     state@options <- 0L
...
#     expect_equal(lapply(slotNames(state), slot, object = state.proc), 
#         lapply(slotNames(state.raw), slot, object = state.raw))
#     expect_error(capture.output(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg())), 
#         type = "message"), "Unable to convert")
#     expect_identical(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg("stats"))), 
#         unitizer:::unitizerStateProcessed(par.env = getNamespace("stats")))
#     expect_error(capture.output(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg("asdfalkdfasd"))), 
#         type = "message"), "Unable to convert")
#     expect_error(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg("")), 
#         test.files = getwd()), "Argument `package` may not be")
...
# expect_error(capture.output(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg())), 
#     type = "message"), "Unable to convert")
capture.output(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg())), 
    type = "message")
# expect_identical(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg("stats"))), 
#     unitizer:::unitizerStateProcessed(par.env = getNamespace("stats")))
...
# expect_error(capture.output(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg("asdfalkdfasd"))), 
#     type = "message"), "Unable to convert")
capture.output(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg("asdfalkdfasd"))), 
    type = "message")
# expect_error(unitizer:::as.state(unitizer:::unitizerStateRaw(par.env = in_pkg("")), 
#     test.files = getwd()), "Argument `package` may not be")
...
#         expect_equal(unitizer:::as.state_raw(my.env), unitizer:::as.unitizerStateRaw(state.proc))
#     })
# })
# - "as.state_raw" -------------------------------------------------------------
...
# test_that("state", {
#     expect_equal(state("stats"), unitizer:::unitizerStateSuggested(par.env = "stats"))
#     expect_equal(state(in_pkg("stats")), unitizer:::unitizerStateSuggested(par.env = in_pkg("stats")))
#     expect_equal(state(in_pkg()), unitizer:::unitizerStateSuggested(par.env = in_pkg()))
#     expect_equal(state(search.path = 1), unitizer:::unitizerStateSuggested(search.path = 1L))
...
#     expect_true(any(grepl("<in: package:stats>", capture.output(show(state(in_pkg("stats")))))))
#     expect_true(any(grepl("namespace:stats", capture.output(show(state(asNamespace("stats")))))))
# })
# - "state" --------------------------------------------------------------------
...
state(search.path = 3)
# expect_error(state(options = 2, namespaces = 1), "is set to 2")
state(options = 2, namespaces = 1)
# expect_error(state(namespaces = 2, search.path = 1), "is set to 2")
state(namespaces = 2, search.path = 1)
state.inv <- unitizer:::unitizerStateProcessed()
state.inv@options <- 2L
# expect_error(unitizer:::as.state(state.inv), "Options state tracking")
unitizer:::as.state(state.inv)
state.inv@namespaces <- 2L
# expect_error(unitizer:::as.state(state.inv), "Namespace state tracking")
unitizer:::as.state(state.inv)
# captured <in: >
...
#     expect_error(unitizer:::in_pkg_to_env(in_pkg(), "/"), "Unable to detect package")
# })
# - "in_pkg" -------------------------------------------------------------------
...
as.character(in_pkg())
# expect_identical(as.character(in_pkg("stats")), "<in: package:stats>")
as.character(in_pkg("stats"))
# expect_identical(capture.output(show(in_pkg())), "<in: auto-detect-pkg>")
capture.output(show(in_pkg()))
# expect_error(unitizer:::in_pkg_to_env(in_pkg(), "/"), "Unable to detect package")
...
#     items <- items + new("unitizerItem", call = quote(1 + 1), 
#         glob.indices = new("unitizerGlobalIndices", search.path = 1L, 
#             options = 2L))
#     items <- items + new("unitizerItem", call = quote(2 + 1), 
#         glob.indices = new("unitizerGlobalIndices", search.path = 2L, 
#             options = 1L))
#     items <- items + new("unitizerItem", call = quote(1 * 1), 
#         reference = TRUE, glob.indices = new("unitizerGlobalIndices", 
#             search.path = 1L, options = 1L))
#     items <- items + new("unitizerItem", call = quote(2 * 1), 
#         reference = TRUE, glob.indices = new("unitizerGlobalIndices", 
#             search.path = 3L, options = 2L))
#     res <- unitizer:::mergeStates(items, trk.new, trk.ref)
#     expect_equal(sapply(res$items, function(x) as.integer(slot(x, 
#         "glob.indices"))), structure(c(1L, 2L, 0L, 0L, 0L, 2L, 
#         1L, 0L, 0L, 0L, 4L, 3L, 0L, 0L, 0L, 5L, 4L, 0L, 0L, 0L), 
#         .Dim = c(5L, 4L), .Dimnames = list(c("search.path", "options", 
#             "working.directory", "random.seed", "namespaces"), 
#             NULL)))
#     s.n.to.check <- c("search.path", "options", "working.directory", 
#         "random.seed", "namespaces")
#     expect_equal(sapply(s.n.to.check, slot, object = res$states), 
#         structure(list(search.path = list(1, 2, 3, 4, 6), options = list("a", 
#             "b", "c", "d"), working.directory = list(), random.seed = list(), 
#             namespaces = list()), .Names = c("search.path", "options", 
#             "working.directory", "random.seed", "namespaces")))
#     items.no.ref <- items[1:2]
#     expect_identical(unitizer:::mergeStates(items.no.ref, trk.new, 
#         trk.ref), list(items = items.no.ref, states = trk.new))
...
#     expect_identical(unitizer:::mergeStates(items.no.new, new("unitizerGlobalTrackingStore"), 
#         trk.ref), list(items = items.no.new.mod, states = trk.ref.mod))
# })
# - "merge states" -------------------------------------------------------------
...
res <- unitizer:::mergeStates(items, trk.new, trk.ref)
# expect_equal(sapply(res$items, function(x) as.integer(slot(x, 
#     "glob.indices"))), structure(c(1L, 2L, 0L, 0L, 0L, 2L, 1L, 
#     0L, 0L, 0L, 4L, 3L, 0L, 0L, 0L, 5L, 4L, 0L, 0L, 0L), .Dim = c(5L, 
#     4L), .Dimnames = list(c("search.path", "options", "working.directory", 
#     "random.seed", "namespaces"), NULL)))
...
# expect_identical(unitizer:::mergeStates(items.no.new, new("unitizerGlobalTrackingStore"), 
#     trk.ref), list(items = items.no.new.mod, states = trk.ref.mod))
unitizer:::mergeStates(items.no.new, new("unitizerGlobalTrackingStore"), 
    trk.ref)
