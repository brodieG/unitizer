...
local({
  ...
  expect_is(sect.2 <- unitizer_sect("mytest", {1 + 1; b <- 5; matrix(integer(), nrow=b, ncol=b)}), "unitizerSectionExpression")
  ...
  expect_is(sect.5 <- unitizer_sect("mytest", expression(1 + 1, b <- 5, matrix(integer(), nrow=b, ncol=b))), "unitizerSectionExpression")
  expect_identical(sect.1, sect.5)
  expect_is(sect.1 <- unitizer_sect("mytest", expr.1, compare=identical), "unitizerSectionExpression")
  ...
  my.unitizer2 <- my.unitizer2 + expr.2                         # now add back items to compare
  my.unitizer3 <- new("unitizer", id=3, zero.env=new.env())
  my.unitizer3 <- my.unitizer3 + my.unitizer@items.new          # make previous items into reference items
  my.unitizer3 <- my.unitizer3 + expr.3                         # now add back items to compare
